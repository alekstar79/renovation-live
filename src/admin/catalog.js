    ...
    columns: [
      {
        ...
        widthMax: 110,
        when: () => true,
        head: () => `<div class="column-head">ID<span class="icon-sort">&#129093</span></div>`,
      ...
      },
      {
        ...
        widthMax: 200,
        when: () => true,
        head: () => `<div class="column-head">Тип услуги<span class="icon-sort">&#129093</span></div>`,
        cell: 'type'
      },
      {
        id: 'name',
        ...
        widthMax: 300,
        when: () => true,
        head: () => `<div class="column-head">Виды работ<span class="icon-sort">&#129093</span></div>`,
        cell: 'name'
      },
      {
        id: 'units',
      ...
      },
      {
        ...
        cell: 'permissions',
        cellSay: () => `<div class="table__actions">` +
...
export const catalog = () => {
  ...
  const cleaningHeader = () => {
    catalogList.shadowRoot.querySelectorAll('.icon-sort')
      .forEach(icon => {
  ...
  const setCurrentData = () => {
    ...
    if (catalogList.records.filterApply.length) {
      catalogList.records.filterRefrech()
    }
    ...
    if (validate()) {
      ...
      if (headerForm.idItem) {
        if (document.taskDemo) {
          ...
          document.dataCatalog.edit(headerForm.idItem, item)
            .then(record => {
              editRecord(record)
            })
        }
  ...
  const deleteCard = () => {
    ...
    if (document.taskDemo) {
      ...
      document.dataCatalog.delete(record.data.id)
        .then(() => {
          deleteCard()
        })
    }
  ...
  const sortCard = (columnHead, mode) => {
    ...
    mode ||= icon.classList.contains('asc') ? 'desc' : icon.classList.contains('desc') ? 'none' : 'asc'
  ...
  form.addEventListener('focusout', (e) => {
    if (e.target.matches('input')) {
      ...
      value = value.replace(valid.multySH, (str, spaces, hyphens) => (spaces ? ' ' : '') + (hyphens ? '-' : ''))
  ...
  card.addEventListener('click', ({ target }) => {
    if (!target.closest('.modal') || target.closest('.button__close') || target.closest('.cancel-button')) {
      card.style.display = 'none'

      // обработка запроса на изменение базы данных
    } else if (target.closest('.button-ui_firm')) {
      submitDatabase()
    }
  })

  // запрос на редактирование / удаление записи / сортировка
  catalogList.shadowRoot.addEventListener('click', ({ target }) => {
    const columnHead = target.closest('.column-head')
  ...
  document.querySelector('.filter_on')
  ...
  document.querySelector('.filter_off')
    .addEventListener('click', () => {
  ...
  document.querySelector('.admin-exit button')
    .addEventListener('click', () => {
