...
export const catalog = () => {
  ...
  const showButton = () => {
    ...
    if (numBlock === navCatalog.childElementCount - 1) {
      rightButton.classList.add('hide')
    }
  }

  // очистка шапки таблицы от ярлыков сортировки
  ...
  const setCurrentData = nameType => {
    catalogList.data = catalog.data.filter(record => record.type === nameType)
  }
  ...
  const orderCatalog = () => {
    ...
    ;[...orderCatalog].sort().forEach((name, index) => {
      htmlTypeItem += `
      <button class="button_o popup-repair-types-nav__item popup__item_${index + 1}">${name}
    ...
    setCurrentBlock(navCatalog.firstElementChild)
  }
  ...
  catalog.addEventListener('click', ({ target }) => {
    ...
    // навигация по блокам
    if ((elem = target.closest('.popup-repair-types-nav__item'))) {
      setCurrentBlock(elem)

      // навигация блоков в мобильной версии
    } else if ((elem = target.closest('.nav-arrow'))) {
  ...
  document.dataCatalog.use()
    .then(data => {
      catalog.data = data // запоминаем данные каталога
      orderCatalog()      // формируем перечень видов работ
    })
}
