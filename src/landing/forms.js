...
import { Picture } from '../modules/helpers.js'
...
export const inputForm = () => {
  ...
  body.addEventListener('focusout', ({ target }) => {
    if (target.closest('.feedback-block__form-input_name')) {
      ...
      value = value.replace(valid.wordFirst, (word, first, f1, other) => first.toUpperCase() + other.toLowerCase())
...
export class SendForm
{
  constructor({ url, errorMessageResponse = 'Ошибка сервера.', optionals = {} })
  {
    ...
    this._dataForm = new DataJSON({ url, errorMessageResponse })
  ...
  send(form)
  {
    ...
    if (validate()) {
          ...
          if ('name' in leading && 'assign' in leading) {
            if ('select' in leading) {
              if ((elem = document.querySelector(leading.select))) {
                formBody[leading.name] = leading.assign in elem
                  ? elem[leading.assign]
                  : leading.assign
              }
            } else {
      ...
      if (!check.checked) {
        check.parentNode.classList.add('error-input')
      }
      if (name && name.value.trim() < 3) {
