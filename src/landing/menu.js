...
import { smoothScroll } from '../modules/helpers.js'
...
export const menu = () => {
  ...
  document.querySelector('body')
    .addEventListener('click', (e) => {
      let itemPopup, toTop, itemMenu = null

      // открытие/закрытие меню
      if (
        e.target.closest('.menu') && !e.target.closest('.menu-phone-icon') ||   // по кнопке открытия меню
        ...
        e.preventDefault()
        smoothScroll(toTop.firstElementChild.getAttribute('href'))
      ...
      if (itemMenu) {
        e.preventDefault()
        smoothScroll(itemMenu.firstElementChild.getAttribute('href'))
      }
      // открытие списка всех услуг
      if (itemPopup) {
        document.modalList.open('popup-repair-types')
      }
    })
