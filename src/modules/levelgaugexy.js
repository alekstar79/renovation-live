...
import { fn, paramDeclaration } from './slt.js'
...
{
  // при создании нового, клонировании importNode() и переносе adoptNode():
  ...
  constructor()
  {
    ...
    frame.append(
      document.createElement('style'), // стили полос уровнемеров
      document.createElement('style'), // стили указателей уровнемеров
      client
    )

    // параметры: изменение атрибута html-элемента из JS
  ...
  connectedCallback()
  {
    ...
    if (!lgxy._isRender) {
      ...
      Object.defineProperty(lgxy, `_isRender`, {
        value: true
      })
    }
  }
  ...
  attributeChangedCallback(name, old, renew)
  {
    ...
    let valid = (key, ask, cur, def) => {
      ...
      if (typeAsk === 'number') {
        ask = Math.max(def, fn.int(ask))

      } else if (typeAsk === 'string') {
        let err = (v) => (!v.includes(ask))

        if (
          (key === 'xSide' && err(['n', 's', 'none', 'none-n', 'none-s'])) ||
    ...
    let validAll = (ask /*, cur */) => {
      ['x', 'y'].forEach((xy) => {
        ...
        let vMax = Math.max(0, ((xy + 'Qty' in ask) ? ask[xy + 'Qty'] : 0) - 1)
  ...
  event(add = true)
  {
    ...
    function down(e, lgxy)
    {
      ...
      let touch = fn.isTouch() ? e.changedTouches[0] : e
      let mX = touch.clientX
      let mY = touch.clientY

      e.preventDefault()

      let client = lgxy.shadowRoot.getElementById('client')
      ...
      // выбранная полоса:
      let isX = xSide > 0
        ? mY > Math.trunc(rect.bottom)
        : mY <= Math.trunc(rect.top)

      let isY = ySide > 0
        ? mX > Math.trunc(rect.right)
        : mX <= Math.trunc(rect.left)
      ...
      function levelChange(mX, mY)
      {
        ...
        if (!ySide || (isX && !isY)) {
          let x = (mX <= rect.left)
            ...
            : (mX >= rect.right - 1)
              ? param.xQty
              : Math.ceil((mX - rect.left) * param.xQty / rect.width)

          param.x = (ySide > -1 ? param.xDepend === 'inverse' : param.xDepend === 'directly')
            ? x - 1
            : param.xQty - x
        }
        ...
        if (!xSide || (isY && !isX)) {
          let y = (mY <= rect.top)
            ...
            : (mY >= rect.bottom - 1)
              ? param.yQty
              : Math.ceil((mY - rect.top) * param.yQty / rect.height)

          param.y = (xSide > -1 ? param.yDepend === 'inverse' : param.yDepend === 'directly')
            ? y - 1
            : param.yQty - y
        }
      }
...
:host {
  ...
  ${xSide > 0 ? `` : `justify-content: flex-end !important;`}
...
:host(:hover) {
  --color-level: var(--color-hover);
    ...
    if (mode.includes('level')) {
      ...
      let y = param.y, yQty = param.yQty //, yStep = param.yStep
