/* tds - Слайдер табличных данных  */

import { paramDeclaration, fn } from './slt.js'
import { LGXY } from './levelgaugexy.js'
...
export const tabularDataSlider = () => {
  ...
  class TDScolumns
  {
    ...
    //                    по умолчанию - по номеру добавлемого столбца
    ...
    add(param = {}, set_colNum = this.list.length, ignoreFixedRight = false)
    {
      ...
      if (!descript.hidden) {
        tds.listX.splice(tds.listX.length - (ignoreFixedRight ? 0 : tds.fixedRight), 0, colNum)
      }

      // обновление содержимого слайдера
      if (tds.xMax > -1) {
        tds.updateData(undefined, undefined, colNum)
      }
      ...
      function description(colNum, param)
      {
        const descript = {
          ...
          when: () => { return false }, // () => {} - предусловие редактирования ячейки данных
    ...
    fixedLeft()
    {
      ...
      if (x < tds.xLeft) {
        ...
        listX.splice(x, tds.xLeft - x)
          .sort((a, b) => a - b)
          .forEach(colNum => {
            listX.splice(x + colsSlide.indexOf(colNum), 0, colNum)
          })

      } else if (x >= tds.xRight) {
        tds.fixedRight -= x - tds.xRight
      }
    ...
    fixedRight()
    {
      ...
      if (x >= tds.xRight) {
        ...
        listX.splice(tds.xRight, x - tds.xRight + 1)
          .sort((a, b) => a - b)
          .forEach(colNum => {
    ...
    hiddenOff(colNum)
    {
      ...
      if (colNum > -1 && colNum < columns.length && !~x) {
        let colsSlide = listX.slice(tds.xLeft, tds.xRight)

        colsSlide.push(colNum)
        colsSlide.sort((a, b) => a - b)
  ...
  class TDSrecords
  {
    ...
    */
    sort(tag, mode = 1, add = false)
    {
      ...
      tag ||= tds.columns.list[tds.listX[tds.x]].id
    ...
    */
      ...
      list.forEach(x => {
        if (x.tag in sortSet) {
          ...
          if (find > -1) {
            ...
            sortApply.push({ tag: x.tag, mode })
          }
        } else {
          console.error(`TDS.records: sortMulty() tag "${x.tag}" не зарегестрирован в sortSet`)
        }
      })
    ...
    sortRefrech()
    {
      const tds = this.tds, listY = tds.listY, sortApply = this.sortApply, sortSet = this.sortSet
      ...
      // формирование таблицы ключей сортировки согласно текущему списку listY
      listY.forEach((recno, i) => {
        ...
        keyData.forEach((init, k) => {
          if (k < keyInfo) {
            ...
            keyData[k].values.push((init.type === 'string') ? v.toUpperCase() : +v)
          }
        })
      })
      ...
      let compare = (i, j) => {
        ...
        for (let k = 0; k < keyInfo; k++) {
          if ((comp = run(keyData[k].values[i[0]], keyData[k].values[j[0]], keyData[k].mode))) {
            break
          }
        }
    ...
    */
    ...
    tag: 'имяТега'
       теги предустановленные с tag == 'tds*',
         ...
         param: {
            colId: "идСтолбца", по умолчанию - id текущего столбца
    ...
    */
    ...
    {
      const tds = this.tds, columns = tds.columns.list, filterSet = this.filterSet, filterApply = this.filterApply
      ...
      list.forEach(x => {
        if (x.tag in filterSet) {
          ...
          if (x.tag.slice(0, 3) === 'tds') {
            ...
            if ('colId' in x.param) {
              colNum = columns.findIndex(col => col.id === x.param.colId)
            } else {
              x.param.colId = columns[colNum].id
            }

            if (colNum < 0) {
              console.error(`TDS.records: filterMulty() столбец с colId "${x.param.colId}" не зарегестрирован в TDS.columns`)
            } else {
          ...
          console.error(`TDS.records: filterMulty() tag "${x.tag}" не зарегестрирован в filterSet`)
        }
      })
      ...
      if (add) {
        return countApp ? this.filterRefrech(countApp) : -1
      } else {
        return countApp ? this.filterRefrech() : (this.filterOff() || -1)
      }
    }
    ...
    */
    ...
    {
      const tds = this.tds, data = tds.data, listY = tds.listY, filterSet = this.filterSet, filterApply = this.filterApply
    ...
    filterOff()
    {
      const tds = this.tds, data = tds.data, listY = tds.listY, recCur = listY[tds.y], filterApply = this.filterApply
  ...
  class TDS extends HTMLElement
  {
    ...
    constructor()
    {
      ...
      ;['heading', 'data', 'footing'].forEach(elName => {
        ...
        ;['left', 'slide', 'right'].forEach((blName, i) => {
          ...
          tds.links[elName + fn.up0(blName)] = rl
        })
      ...
      tds.resizeObserver = new window.ResizeObserver(entries => {
        ...
        entries.forEach((entry) => {
          ...
          if (tds.render > 0) { // встроено с use||add
            ...
            if (dWidth || dHeight) {
              window.setTimeout(() => tds.resize(width, height), 0)
            }
          }
        })
      })
    ...
    connectedCallback()
    {
      ...
      if (tds.render < 0) { // не встрено
        ...
        const gCSv = mask => gCS.getPropertyValue(mask).replace(re, '')
    ...
    attributeChangedCallback(name, old, renew)
    {
      ...
      let valid = (key, ask, cur, def) => {
        if (typeof (ask) === 'number') {
          ask = Math.max(fn.int(ask), ((key.includes('gap') || key.includes('Height')) ? 0 : def))

        } else {
          if (
            (key === 'lgxy' && !['auto', 'visible', 'hidden'].includes(ask)) ||
      ...
      if ('gapCol' in attrRenew) {
        stInit = stCol = true
      }
      if ('gapRow' in attrRenew || 'rowHeight' in attrRenew) {
        stInit = reRow = true
      }
      if ('headingHeight' in attrRenew || 'footingHeight' in attrRenew || 'divisionLine' in attrRenew) {
        stInit = true
      }
    ...
    set row(value) { this.paramRoll.row = (value < 0 || this.rowMax < 0) ? 0 : (value > this.rowMax) ? this.rowMax : value }
    ...
    use(initData)
    {
      ...
      // - согласно описаниям в объекте инициализации .colunms:
      if ('columns' in initData && initData.columns.length > 0) {
        ...
        // - согласно структуре первой строки json-данных описаний
    ...
    controlRow(cohere = true, dHeight = 0)
    {
      ...
      function cohereWithDom()
      {
        ...
        if (dRows > 0 || dHeight > 0) {
          ...
          if (curLineBottomEdge >= tdsHeight) {
            top = preHideRow
          }
        }
    ...
    controlCol(cohere = true, set_colNum = -1, dWidth = 0)
    {
      ...
      function cohereWithDom()
      {
        ...
        // целевой логический образ расположения колонок согласно порядку listX
        let imageCreation = (cols, qtyL, qtyR) => {
          ...
          for (let i = qty - qtyR; i < qty; i++) {
            img.Right.push(cols[i])
          }

          img.Right.sort((a, b) => a - b)

          for (let i = 0; i < qtyFull; i++) { // в том числе скрытые
            if (!img.Left.includes(i) && !img.Right.includes(i)) {
              img.Slide.push(i)
            }
          }
        ...
        let target = imageCreation(listX, fixedLeft, fixedRight)
          ...
          for (let pos = 0; pos < dom[sele].length; pos++) {
            ...
            if (shadow !== setShadow || hidden !== setHidden) {
              cellsCol(sele, pos)
                .forEach(col => {
                  if (shadow && !setShadow) {
                    col.classList.add(stS)
                  } else if (!shadow && setShadow) {
                    col.classList.remove(stS)
                  }
                  if (hidden && !setHidden) {
                    col.hidden = true
                  } else if (!hidden && setHidden) {
                    col.hidden = false
                  }
                })
            }
        ...
        let addListMove = (mode, fS, fP, tS, tP) => {
          listMove.push({ mode, fromSele: fS, fromPos: fP, toSele: tS, toPos: tP })
        }
          ...
          for (let fromPos = 0; fromPos < dom[fromSele].length; fromPos++) {
            ...
            if (!target[fromSele].includes(colNum)) {
              ...
              let toSele = target[seekSele[0]].includes(colNum)
                ? seekSele[0]
                : target[seekSele[1]].includes(colNum)
                  ? seekSele[1]
                  : ''
              ...
              if (toSele) { // не скрытые в Slide
                ...
                if (~toPos) {
                  dom[toSele].splice(toPos, 0, colNum)
                } else {
                  dom[toSele].push(colNum)
                }
        ...
        listMove.forEach(move => {
          let [mode, fromSele, fromPos, toSele, toPos] = [move.mode, move.fromSele, move.fromPos, move.toSele, move.toPos]
        ...
        if (tds.xMax - listX.length + 1) {
          tds.xMax = listX.length - 1
        }
      }
    }
    ...
    cursor()
    {
      ...
      let sele = x < tds.xLeft ? 'dataLeft' : x < tds.xRight ? 'dataSlide' : 'dataRight'
      ...
      if (document.activeElement === tds) {
        cell.classList.add('-tds-active')
      }

      // установка смещения roll:
      if (tds.row === 0 && tds.rowMax > 0) {
        tds.paramRoll.top = 0
      } else if (tds.row === tds.rowMax) {
        tds.paramRoll.top = Math.max(tds.paramRoll.hideRow, 0)
      }
    ...
    event(add = true)
    {
      if (add && !(this._eventList)) {
        ...
        Object.defineProperty(this, `_eventList`, {
          value: Object.create({}, {
            ...
            down: { value: e => down(e, tds, (fn.isTouch() ? 'touchstart' : 'mousedown')) },
            ...
            focus: {
              value: () => {
                ...
                if (cell && !cell.classList.contains('-tds-active')) {
                  cell.classList.add('-tds-active') // фокус приобрел
                }
              }
            },
            blur: {
              value: () => {
                ...
                if (cell && cell.classList.contains('-tds-active')) {
                  cell.classList.remove('-tds-active') // фокус потерял
                }
              }
            }
          })
      ...
      this[act]((fn.isTouch() ? 'touchstart' : 'mousedown'), event.down, false)
      ...
      function eventLGXY(e, tds)
      {
        ...
        if ((refrech = 'yQty' in e.detail)) {
          ...
          let update = (bof && !top) || (eof && !bottom) || tds.row + skip < 0 || tds.row + skip > tds.rowMax
        ...
        switch (key) {
          ...
          case 'Home':
            if (e.ctrlKey) { // на первый столбец первой строки
              ...
              tds.x = 0      // на первый столбец текущей строки
            }
            break
          case 'End':
            if (e.ctrlKey) { // на последний столбец последней строки
              ...
              tds.x = tds.xMax // на последний столбец текущей строки
            }
            break
      ...
      function down(e, tds, type)
      {
        ...
        if (!fn.isTouch() && e.which > 1) return // только левая кнопка мыши
        ...
        if (cl.includes('-tds-heading-col')) {
          ...
          if (x !== tds.x && skip) {
            tds.lgxy.param.group = { x: x, y: tds.y + skip }
          } else if (x !== tds.x) {
            tds.x = x
          } else if (skip) {
            tds.y += skip
    ...
    updateStyle(mode = 'init+cols+scroll-col+scroll-row')
    {
      ...
      if (mode.includes('init')) {
        ...
        const cssCursor = paramRoll.cssCursor   || 'outline: 1px solid rgb(80,135,215); outline-offset: -1px;'
...
.-tds-type-object  {}
      ...
      if (mode.includes('cols')) {
        ...
        for (let x = 0; x < listX.length; x++) {
          ...
          style += `.-tds-cell[data-col="${colNum}"] { grid-area: col${colNum}; }`;
        }
        ...
        if (tds.param.lgxy === 'auto' && !tds.lgxy.param.xSide.includes('none')) {
          ...
          if (xHide !== tds.lgxy.param.xHide) {
            tds.lgxy.param.xHide = xHide
          }
        }
      ...
      if (mode.includes('scroll') && tds.render > 0) {
        ...
        if (mode.includes('scroll-row')) {
          ...
          if (tds.param.lgxy === 'auto' && !tds.lgxy.param.ySide.includes('none')) {
            ...
            if (yHide !== tds.lgxy.param.yHide) {
              tds.lgxy.param.yHide = yHide
            }
          }

          modeStyle[2].textContent = `.-tds-data .-tds-roll { top: ${-paramRoll.top}px; }`
        }
        ...
        if (mode.includes('scroll-col') && links.currentCell) {
          ...
          modeStyle[3].textContent = `.-tds-slide .-tds-roll { left: ${-left}px; }`
        }
      }
    }
  }
