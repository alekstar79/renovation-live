...
export class Picture
{
  ...
  start(value)
  {
    this._buffer = value.replace(/\D/g, '').slice(0, this._mapBuffer.length).padEnd(this._mapBuffer.length, this._placeholder)
    this._value = this._mask.split('')
  ...
  edit(input, type)
  {
    ...
    if (!type) {
      input.value = this.start(input.value)
    }

    const deletion = (bufferPosition) => {
      const buffer = this._buffer

      // удаляем символ справа от курсора, последующие символы сдвигаем влево и дополняем маской
      this._buffer = ((bufferPosition ? buffer.slice(0, bufferPosition) : '') + buffer.slice(bufferPosition + 1, buffer.length))
    ...
    if (type === 'insertText') {
      ...
      if (~bufferPosition) {
        ...
        // переформировываем маску
        this._mapBuffer.forEach((pos, index) => {
          this._value.splice(pos, 1, this._buffer.slice(index, index + 1))
        })
...
export const animate = ({ draw, duration = 1000, timingplane = 'linear' }) => {
  ...
  requestAnimationFrame(function animation() {
    // вычисление текущего состояния анимации число от 0 до 1
    // с учетом указанной линейности, заданной в настроке timing
