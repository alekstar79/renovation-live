...
export const fn = {
  ...
  toCamel: s => s.replace(/-[a-z]+/g, s => fn.up0(s.slice(1))),
  ...
  styleToJS: s => fn.noComments(s)
    .replace(/\s+/g, '')
    .split(';')
    .reduce((js, c) => {
      if (c) {
        let st = c.split(':')
        js[fn.toCamel(st[0])] = st[1] || ''
      }
...
export class paramDeclaration
{
  constructor(defaultList, parent)
  {
    ...
    for (let para in defaultList) {
      ...
      Object.defineProperty(this, key, {
        ...
        get()
        {
          return (key in this.parent._dataList) ? this.parent._dataList[key] : defaultValue
        },
        set(v)
        {
          this.group = { [key]: v }
  ...
  set text(v)
  {
    this.parent.attributeChangedCallback('param', this.text, v)
  }
  ...
  get text()
  {
    ...
    Object.keys(this).filter(key => !isNaN(+key)).forEach((key) => {
      text += `${fn.ofCamel(this[key])}: ${this.parent._dataList[this[key]]}; `
    })
  ...
  set group(v)
  {
    this.parent.attributeChangedCallback('param', this.text, fn.jsToStyle(Object.assign({}, this.group, v)))
  }
  ...
  get group()
  {
    ...
    Object.keys(this)
      ...
      .forEach((key) => {
        Object.assign(group, {
          [this[key]]: this.parent._dataList[this[key]]
        })
      })
  ...
  changedCallback(renew, valid, validAll)
  {
    ...
    let attrAsk = renew ? fn.styleToJS(renew) : {}
    ...
    if (validAll) {
      validAll(attrNew, Object.assign({}, paramCur))
    }
    ...
    for (let key in attrNew) {
      if (attrNew[key] !== null) {
        Object.defineProperty(this, `${i}`, { value: key, enumerable: true, configurable: true })

        i++

        Object.defineProperty(_dataList, `${key}`, { value: attrNew[key], configurable: true })
      }
    }
    ...
    for (let key in paramCur) {
      ...
      let vAsk = (key in attrAsk)   // запрашивали
        ? attrAsk[key]
        : vOld
